...
x_rms_thermal = np.sqrt(kB * T / kx) if kx > 0 else np.nan
y_rms_thermal = np.sqrt(kB * T / ky) if ky > 0 else np.nan
z_rms_thermal = np.sqrt(kB * T / kz) if kz > 0 else np.nan
...
def brownian_force_rms_fdt(damping_b):
    ...
    return np.sqrt(2 * damping_b * kB * T / dt_baoab)
...
if run_multiple_seed_validation:
    ...
    ax.set_ylabel(r"simulated RMS / $\sqrt{k_B T / k}$")
