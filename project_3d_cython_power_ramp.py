...
try:
    try:
        import baoab_3d_cython_pressure_ramp as _baoab_3d_cython
    except ImportError:
    ...
    solve_baoab_3d_pressure_lookup_cython = getattr(
        _baoab_3d_cython,
...
truncate_power_ramp_plot_at_lookup_exit = True
...
stability_rms_step_seconds = 1.0
...
force_lookup_Fz_ray_table = None
force_lookup_Fz_photo_base_table = None
...
def build_force_lookup_table(r_min, r_max, z_min, z_max):
    ...
    global force_lookup_Fz_ray_table
    global force_lookup_Fz_photo_base_table
    ...
    Fz_ray_table = np.zeros_like(Fr_table)
    Fz_photo_base_table = np.zeros_like(Fr_table)

    for ir, r_value in enumerate(force_lookup_r_values):
        for iz, z_value in enumerate(force_lookup_z_values):
            Fr_ray, Fz_ray = F_beam_2d(
                r_value,
                z_value,
                power_factor=1.0
            )
            _, Fz_photo_base = F_photo_2d(
                r_value,
                z_value,
                power_factor=1.0
            )

            Fr_table[ir, iz] = Fr_ray
            Fz_table[ir, iz] = Fz_ray + Fz_photo_base
            Fz_ray_table[ir, iz] = Fz_ray
            Fz_photo_base_table[ir, iz] = Fz_photo_base
    ...
    force_lookup_Fz_ray_table = np.ascontiguousarray(Fz_ray_table, dtype=np.float64)
    force_lookup_Fz_photo_base_table = np.ascontiguousarray(
        Fz_photo_base_table,
        dtype=np.float64
    )
...
def photophoretic_pressure_scale(pressure, reference_pressure=p):
    ...
    reference_response = 2.0 / (
        (reference_pressure / p_max_ph) + (p_max_ph / reference_pressure)
    )
    pressure_response = 2.0 / (
        (pressure_arr / p_max_ph) + (p_max_ph / pressure_arr)
    )
    return pressure_response / reference_response


def power_ramp(time_values):
    if power_ramp_start <= 0 or power_ramp_end <= 0:
        raise ValueError("Power ramp endpoints must be positive.")

    elapsed_time = np.asarray(time_values, dtype=float) - time_values[0]
    ramp_fraction = elapsed_time / (time_values[-1] - time_values[0])
    ramp_fraction = np.clip(ramp_fraction, 0.0, 1.0)

    if power_ramp_profile == "linear":
        return power_ramp_start + ramp_fraction * (
            power_ramp_end - power_ramp_start
        )

    if power_ramp_profile == "log":
        log_start = np.log(power_ramp_start)
        log_end = np.log(power_ramp_end)
        return np.exp(log_start + ramp_fraction * (log_end - log_start))

    raise ValueError("power_ramp_profile must be 'linear' or 'log'.")


def rolling_window_indices(time_values, window_seconds, step_seconds):
    if window_seconds <= 0.0:
        raise ValueError("stability_rms_window_seconds must be positive.")
    if step_seconds <= 0.0:
        raise ValueError("stability_rms_step_seconds must be positive.")

    dt_local = float(np.median(np.diff(time_values)))
    window_samples = max(2, int(round(window_seconds / dt_local)))
    step_samples = max(1, int(round(step_seconds / dt_local)))

    if window_samples > len(time_values):
        raise ValueError(
            "The rolling RMS window is longer than the simulated trajectory."
        )

    starts = np.arange(0, len(time_values) - window_samples + 1, step_samples)
    ends = starts + window_samples
    centre_times = 0.5 * (time_values[starts] + time_values[ends - 1])

    return starts, ends, centre_times


def rolling_rms_from_deviation(deviation, starts, ends):
    finite = np.isfinite(deviation)
    squared = np.where(finite, deviation**2, 0.0)
    cumulative_squared = np.concatenate(([0.0], np.cumsum(squared)))
    cumulative_count = np.concatenate(([0], np.cumsum(finite.astype(int))))

    window_counts = cumulative_count[ends] - cumulative_count[starts]
    expected_counts = ends - starts
    window_sums = cumulative_squared[ends] - cumulative_squared[starts]

    rms = np.full(len(starts), np.nan)
    valid = window_counts == expected_counts
    rms[valid] = np.sqrt(window_sums[valid] / window_counts[valid])

    return rms


def interpolate_finite_values(source_times, source_values, target_times):
    finite = np.isfinite(source_values)
    interpolated = np.full_like(target_times, np.nan, dtype=float)

    if np.count_nonzero(finite) >= 2:
        finite_times = source_times[finite]
        finite_values = source_values[finite]
        interpolation_mask = (
            (target_times >= finite_times[0])
            & (target_times <= finite_times[-1])
        )
        interpolated[interpolation_mask] = np.interp(
            target_times[interpolation_mask],
            finite_times,
            finite_values
        )
    elif np.count_nonzero(finite) == 1:
        interpolated[:] = source_values[finite][0]

    return interpolated


def stable_equilibrium_and_springs_at_power_factor(
    power_factor_value,
    previous_z=None
):
    def Fz_net_on_axis_at_power(z):
        _, Fz = F_optical_2d(0.0, z, power_factor_value)
        return Fz - m*g

    root = None
    if previous_z is not None and np.isfinite(previous_z):
        search_limit = equilibrium_z_max - equilibrium_z_min
        for half_width in np.geomspace(1e-7, search_limit, 48):
            z_lower = max(equilibrium_z_min, previous_z - half_width)
            z_upper = min(equilibrium_z_max, previous_z + half_width)
            if z_lower >= z_upper:
                continue

            F_lower = Fz_net_on_axis_at_power(z_lower)
            F_upper = Fz_net_on_axis_at_power(z_upper)
            if F_lower == 0.0:
                root = z_lower
                break
            if F_upper == 0.0:
    ...
    if root is None:
        ...
        for i in range(len(z_scan) - 1):
            if F_scan_power[i] * F_scan_power[i + 1] < 0:
                ...
                roots_power.append(candidate_root)

        stable_roots_power = []
        for candidate_root in roots_power:
            slope = numerical_derivative_1d(
                Fz_net_on_axis_at_power,
                candidate_root
            )
            if slope < 0.0:
                stable_roots_power.append(candidate_root)
        ...
        root = stable_roots_power[equilibrium_root_index]

    def Fx_at_x_power(x):
        Fx, _ = F_optical_2d(x, root, power_factor_value)
        return Fx

    kx_power = -numerical_derivative_1d(Fx_at_x_power, x_eq)
    ky_power = kx_power
    kz_power = -numerical_derivative_1d(Fz_net_on_axis_at_power, root)

    if kx_power <= 0.0 or kz_power <= 0.0:
        return np.nan, np.nan, np.nan, np.nan

    return root, kx_power, ky_power, kz_power


def damping_coefficient_array(pressure_values, model="auto"):
    pressure_arr = np.asarray(pressure_values, dtype=float)
    if np.any(pressure_arr <= 0):
        raise ValueError("Pressure values must be positive.")

    stokes_b = damping_coefficient_stokes()
    Kn = mean_free_path(pressure_arr) / radius
    cunningham_b = stokes_b / (
        1 + Kn * (cunningham_A + cunningham_B * np.exp(-cunningham_C / Kn))
    )

    rho = gas_density(pressure_arr)
    c_bar = mean_thermal_speed()
    accommodation_factor = 1 + np.pi * epstein_accommodation_alpha / 8
    epstein_b = (
        (4 / 3)
    ...
    if model == "auto":
        return np.where(Kn < 0.1, stokes_b, np.where(Kn < 10, cunningham_b, epstein_b))
    if model == "stokes":
        return np.full_like(pressure_arr, stokes_b, dtype=float)
    if model == "cunningham":
        return cunningham_b
    if model == "epstein":
        return epstein_b
...
def solve_baoab_3d_fast_with_pressure_ramp(
    power_factor_time,
    photo_pressure_factor_time,
    damping_factor_time,
    thermal_velocity_scale_time,
    brownian_normals_x,
    brownian_normals_y,
    brownian_normals_z,
):
...
def first_force_lookup_exit(x_values, y_values, z_values, chunk_size=1_000_000):
    """
    Return the first sample outside the force-lookup domain and why, or None.
    """
    for start_index in range(0, len(z_values), chunk_size):
        stop_index = min(start_index + chunk_size, len(z_values))
        radial_displacement = np.hypot(
            x_values[start_index:stop_index],
            y_values[start_index:stop_index],
        )
        z_chunk = z_values[start_index:stop_index]
        radial_outside = radial_displacement > force_lookup_r_values[-1]
        z_below = z_chunk < force_lookup_z_values[0]
        z_above = z_chunk > force_lookup_z_values[-1]
        outside_lookup = radial_outside | z_below | z_above
        outside_indices = np.flatnonzero(outside_lookup)
        if outside_indices.size > 0:
            local_index = int(outside_indices[0])
            sample_index = start_index + local_index
            if radial_outside[local_index]:
                reason = (
                    "radial displacement "
                    f"{radial_displacement[local_index] * 1e6:.3g} micrometres "
                    f"exceeded lookup r max {force_lookup_r_values[-1] * 1e6:.3g} micrometres"
                )
            elif z_below[local_index]:
                reason = (
                    "z position "
                    f"{z_chunk[local_index] * 1e6:.3g} micrometres "
                    f"fell below lookup z min {force_lookup_z_values[0] * 1e6:.3g} micrometres"
                )
            else:
                reason = (
                    "z position "
                    f"{z_chunk[local_index] * 1e6:.3g} micrometres "
                    f"exceeded lookup z max {force_lookup_z_values[-1] * 1e6:.3g} micrometres"
                )

            return sample_index, reason
...
pressure_time = np.full_like(t_baoab, p, dtype=float)
pressure_step_time = pressure_time[:-1]
photo_pressure_factor_time = photophoretic_pressure_scale(
    pressure_step_time,
    p
)
pressure_b_time = damping_coefficient_array(pressure_step_time, drag_model)
pressure_gamma_time = pressure_b_time / m
pressure_baoab_damping_factor_time = np.exp(-pressure_gamma_time * dt_baoab)
pressure_baoab_thermal_velocity_scale_time = np.sqrt(
    np.maximum(
        0.0,
        (kB * T / m) * (1 - pressure_baoab_damping_factor_time**2)
    )
)

print("Constant pressure =", pressure_time[0], "Pa")
print("Power ramp start =", power_time[0] * 1e3, "mW")
print("Power ramp end =", power_time[-1] * 1e3, "mW")
print("Power ramp duration =", t_baoab[-1] - t_baoab[0], "s")
print("Initial damping gamma =", pressure_gamma_time[0], "s^-1")
print("Final damping gamma =", pressure_gamma_time[-1], "s^-1")

power_ramp_runtime_start = perf_counter()
(
    x_baoab_power_ramp,
    y_baoab_power_ramp,
    z_baoab_power_ramp,
    ...
    vy_baoab_power_ramp,
    vz_baoab_power_ramp,
    power_ramp_out_of_bounds_count,
) = solve_baoab_3d_fast_with_pressure_ramp(
    power_factor_time,
    photo_pressure_factor_time,
    pressure_baoab_damping_factor_time,
    pressure_baoab_thermal_velocity_scale_time,
    brownian_normals_x,
    brownian_normals_y,
    brownian_normals_z,
)
power_ramp_runtime = perf_counter() - power_ramp_runtime_start
print("Power-ramp BAOAB runtime =", power_ramp_runtime, "s")

lookup_exit = first_force_lookup_exit(
    x_baoab_power_ramp,
    y_baoab_power_ramp,
    z_baoab_power_ramp,
)
lookup_exit_index = None if lookup_exit is None else lookup_exit[0]
plot_sample_count = len(t_baoab)

if power_ramp_out_of_bounds_count > 0:
    print(
        "Warning: power-ramp Cython force lookup clamped",
        power_ramp_out_of_bounds_count,
        "force evaluations."
    )

if lookup_exit_index is not None:
    print(
        "Warning: trajectory left the force lookup at t =",
        t_baoab[lookup_exit_index],
        "s, power =",
        power_time[lookup_exit_index] * 1e3,
        "mW;",
        lookup_exit[1] + ".",
        "Plotting only the trusted interval."
    )
    if truncate_power_ramp_plot_at_lookup_exit:
        plot_sample_count = lookup_exit_index + 1

trusted_slice = slice(None, plot_sample_count)
max_trusted_z_position_um = np.max(
    np.abs(z_baoab_power_ramp[trusted_slice])
) * 1e6
...
plot_stride = max(1, plot_sample_count // max_plot_points)
plot_slice = slice(None, None, plot_stride)
t_plot = t_baoab[trusted_slice][plot_slice]
power_plot_stride = max(1, len(t_baoab) // max_plot_points)
power_plot_slice = slice(None, None, power_plot_stride)
power_t_plot = t_baoab[power_plot_slice]
power_plot_mw = power_time[power_plot_slice] * 1e3

fig, position_axis = plt.subplots(figsize=time_trace_figsize)
power_axis = position_axis.twinx()
...
power_axis.set_ylabel("laser power (mW)")
power_min_mw = min(power_ramp_start, power_ramp_end) * 1e3
power_max_mw = max(power_ramp_start, power_ramp_end) * 1e3
power_padding_mw = 0.05 * (power_max_mw - power_min_mw)
power_axis.set_ylim(
    power_min_mw - power_padding_mw,
    power_max_mw + power_padding_mw
)
position_axis.set_title("Particle trajectory during laser-power ramp")
...
position_axis.legend(
    position_lines + power_lines,
    position_labels + power_labels,
    loc="best"
)
fig.tight_layout()
finish_plot()
...
if include_power_ramp_stability_plot:
    print(
        ...
        stability_rms_window_seconds,
        "s windows every",
        stability_rms_step_seconds,
        "s."
    )
    rms_starts, rms_ends, rms_times = rolling_window_indices(
        t_baoab,
        stability_rms_window_seconds,
        stability_rms_step_seconds
    )
    equilibrium_sample_times = np.unique(
        np.concatenate(([t_baoab[0]], rms_times, [t_baoab[-1]]))
    )
    equilibrium_power_factor_time = np.interp(
        equilibrium_sample_times,
        t_baoab,
        power_factor_time
    )

    z_eq_sample_time = np.full_like(equilibrium_sample_times, np.nan, dtype=float)
    kx_sample_time = np.full_like(equilibrium_sample_times, np.nan, dtype=float)
    ky_sample_time = np.full_like(equilibrium_sample_times, np.nan, dtype=float)
    kz_sample_time = np.full_like(equilibrium_sample_times, np.nan, dtype=float)

    previous_z_eq = z_eq
    for i, power_factor_value in enumerate(equilibrium_power_factor_time):
        (
            z_eq_power,
            kx_power,
            ky_power,
            kz_power,
        ) = stable_equilibrium_and_springs_at_power_factor(
            power_factor_value,
            previous_z_eq
        )
        z_eq_sample_time[i] = z_eq_power
        kx_sample_time[i] = kx_power
        ky_sample_time[i] = ky_power
        kz_sample_time[i] = kz_power
        if np.isfinite(z_eq_power):
            previous_z_eq = z_eq_power

    if np.count_nonzero(np.isfinite(z_eq_sample_time)) == 0:
        print(
    ...
    kx_rms_time = interpolate_finite_values(
        equilibrium_sample_times,
        kx_sample_time,
        rms_times
    )
    ky_rms_time = interpolate_finite_values(
        equilibrium_sample_times,
        ky_sample_time,
        rms_times
    )
    kz_rms_time = interpolate_finite_values(
        equilibrium_sample_times,
        kz_sample_time,
        rms_times
    )

    x_deviation = x_baoab_power_ramp - x_eq
    y_deviation = y_baoab_power_ramp - y_eq
    z_deviation = z_baoab_power_ramp - z_eq_time
    horizontal_deviation = np.sqrt(x_deviation**2 + y_deviation**2)

    x_rolling_rms = rolling_rms_from_deviation(
        x_deviation,
        rms_starts,
        rms_ends
    )
    y_rolling_rms = rolling_rms_from_deviation(
        y_deviation,
        rms_starts,
        rms_ends
    )
    horizontal_rolling_rms = rolling_rms_from_deviation(
        horizontal_deviation,
        rms_starts,
        rms_ends
    )
    z_rolling_rms = rolling_rms_from_deviation(
        z_deviation,
        rms_starts,
        rms_ends
    )

    x_thermal_rms_time = np.sqrt(kB * T / kx_rms_time)
    y_thermal_rms_time = np.sqrt(kB * T / ky_rms_time)
    horizontal_thermal_rms_time = np.sqrt(
        x_thermal_rms_time**2 + y_thermal_rms_time**2
    )
    z_thermal_rms_time = np.sqrt(kB * T / kz_rms_time)

    fig, rms_axis = plt.subplots(figsize=single_diagnostic_figsize)
    power_axis = rms_axis.twinx()

    rms_axis.plot(
        rms_times,
        horizontal_rolling_rms * 1e6,
        color="tab:blue",
        linewidth=1.3,
        label="horizontal rolling RMS"
    )
    rms_axis.plot(
        rms_times,
        z_rolling_rms * 1e6,
    ...
    rms_axis.plot(
        rms_times,
        horizontal_thermal_rms_time * 1e6,
        color="tab:blue",
        linestyle="--",
        linewidth=1.0,
        label="horizontal thermal RMS"
    )
    rms_axis.plot(
        rms_times,
        z_thermal_rms_time * 1e6,
        color="tab:orange",
        linestyle="--",
        linewidth=1.0,
        label="vertical thermal RMS"
    )
    power_axis.plot(
        power_t_plot,
        power_plot_mw,
    ...
    rms_axis.set_xlabel("time (s)")
    rms_axis.set_ylabel("rolling RMS displacement (micrometres)")
    power_axis.set_ylabel("laser power (mW)")
    rms_axis.set_xlim(t_start, t_end)
    power_axis.set_ylim(
        power_min_mw - power_padding_mw,
        power_max_mw + power_padding_mw
    )
    rms_axis.set_title(
        f"Trap stability during power ramp ({stability_rms_window_seconds:g} s RMS window)"
    )
