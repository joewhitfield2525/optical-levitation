...
save_path = CYTHON_DIR / "pressure_ramp"
...
pressure_ramp_mid = p
pressure_ramp_end = 100
pressure_ramp_stage_duration = 200.0
...
dt_baoab = 1 / 30000
max_python_fallback_samples = 2_000_000
brownian_seed = 900
...
def F_optical_3d_lookup_scalar_clamped_pressure_scaled(
    ...
    The lookup table stores axial ray-optics and photophoretic terms
    separately, so the pressure-ramp fallback can scale the photophoretic force
    at each timestep without rebuilding the table.
    """
    r = np.sqrt(x**2 + y**2)
...
def pressure_ramp(time_values):
    if pressure_ramp_start <= 0 or pressure_ramp_mid <= 0 or pressure_ramp_end <= 0:
        raise ValueError("Pressure ramp endpoints must be positive.")

    elapsed_time = np.asarray(time_values, dtype=float) - time_values[0]

    if pressure_ramp_profile == "linear":
        ramp_fraction = elapsed_time / (time_values[-1] - time_values[0])
        ramp_fraction = np.clip(ramp_fraction, 0.0, 1.0)
        return pressure_ramp_start + ramp_fraction * (
            pressure_ramp_end - pressure_ramp_start
        )
    if pressure_ramp_profile == "two_stage_linear":
        first_stage_fraction = np.clip(
            elapsed_time / pressure_ramp_stage_duration,
            0.0,
            1.0
        )
        second_stage_fraction = np.clip(
            (elapsed_time - pressure_ramp_stage_duration)
            / pressure_ramp_stage_duration,
            0.0,
            1.0
        )
        return np.where(
            elapsed_time <= pressure_ramp_stage_duration,
            pressure_ramp_start
            + first_stage_fraction * (pressure_ramp_mid - pressure_ramp_start),
            pressure_ramp_mid
            + second_stage_fraction * (pressure_ramp_end - pressure_ramp_mid),
        )
    if pressure_ramp_profile == "log":
        ramp_fraction = elapsed_time / (time_values[-1] - time_values[0])
        ramp_fraction = np.clip(ramp_fraction, 0.0, 1.0)
        log_start = np.log(pressure_ramp_start)
        log_end = np.log(pressure_ramp_end)
        return np.exp(log_start + ramp_fraction * (log_end - log_start))

    raise ValueError(
        "pressure_ramp_profile must be 'linear', 'two_stage_linear', or 'log'."
    )
...
# Time range
# ****************************************************************************************************************************************************
estimated_sample_count = int(np.floor((t_end - t_start) / dt_baoab)) + 1
if not cython_pressure_baoab_available and estimated_sample_count > max_python_fallback_samples:
    old_dt_baoab = dt_baoab
    dt_baoab = (t_end - t_start) / (max_python_fallback_samples - 1)
    print(
        "Cython pressure-ramp solver unavailable; increasing dt_baoab from",
        old_dt_baoab,
        "s to",
        dt_baoab,
        "s so the Python fallback uses at most",
        max_python_fallback_samples,
        "samples."
    )
...
def solve_baoab_3d_fast_with_pressure_ramp(
    ...
    """
    BAOAB loop for a time-varying gas pressure.

    Uses the Cython pressure-ramp solver when available. If that local Cython
    extension has not been built, falls back to a slower pure-Python loop.
    """
    if terminate_on_trap_loss:
        raise RuntimeError(
    ...
    if not (
        ...
        raise RuntimeError(
            "Pressure-ramp mode requires use_force_lookup_table=True and a force "
...
print("Pressure ramp start =", pressure_time[0], "Pa")
print("Pressure ramp end =", pressure_time[-1], "Pa")
print("Pressure ramp duration =", t_baoab[-1] - t_baoab[0], "s")
print("Initial ramp damping gamma =", pressure_gamma_time[0], "s^-1")
print("Final ramp damping gamma =", pressure_gamma_time[-1], "s^-1")

pressure_ramp_start_time = perf_counter()
(
    x_baoab_pressure_ramp,
    y_baoab_pressure_ramp,
    z_baoab_pressure_ramp,
    vx_baoab_pressure_ramp,
    vy_baoab_pressure_ramp,
    vz_baoab_pressure_ramp,
    pressure_ramp_out_of_bounds_count,
...
pressure_ramp_runtime = perf_counter() - pressure_ramp_start_time
print("Pressure-ramp BAOAB runtime =", pressure_ramp_runtime, "s")
...
if pressure_ramp_out_of_bounds_count > 0:
    print(
        "Warning: pressure-ramp Cython force lookup clamped",
        pressure_ramp_out_of_bounds_count,
        "force evaluations."
    )
...
pressure_plot_stride = max(1, len(t_baoab) // max_plot_points)
