...
# -------------------------------------------------------------------------
# For this single simulated trajectory, this is the squared displacement from
# the initial equilibrium position. The t = 0 point is omitted because log axes
# cannot display zero.
msd_single_trajectory = (x - x[0])**2
positive_time_mask = t > 0

plt.figure(figsize=(9, 5))
plt.loglog(
    t[positive_time_mask],
    msd_single_trajectory[positive_time_mask] * 1e18,
    linewidth=0.9,
    label="single-trajectory MSD"
)
plt.xlabel("Time / s")
