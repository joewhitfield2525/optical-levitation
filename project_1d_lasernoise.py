...
# where:
#
#     F_det(z) = F_scat(z) + F_grad(z) + F_photo(z,p) - mg
#
...
z_baoab = np.zeros_like(t_baoab)
v_baoab = np.zeros_like(t_baoab)
...
for i in range(len(t_baoab) - 1):
    z_i = z_baoab[i]
    v_i = v_baoab[i]
    power_factor_i = laser_power_factor[i]
    ...
    v_i = (
        ...
        + baoab_thermal_velocity_scale * rng.normal()
    )
