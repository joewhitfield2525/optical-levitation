...
zR_manual = 100e-6
P_laser = 0.215             # W, nominal/mean laser power
use_laser_power_noise = True
laser_noise_trace_path = Path(
...
def load_power_meter_trace(csv_path):
    csv_path = Path(csv_path)

    with csv_path.open("r", encoding="utf-8-sig", newline="") as handle:
        lines = handle.readlines()

    graph_header_index = None

    for index, line in enumerate(lines):
        if line.strip().lower() == "time (ms),power (w)":
            graph_header_index = index
            break

    if graph_header_index is None:
        raise ValueError(
            f"Could not find 'Time (ms),Power (W)' data header in {csv_path}."
        )

    trace_data = np.genfromtxt(
        lines[graph_header_index + 1:],
        delimiter=","
    )
    ...
    if trace_data.size == 0 or trace_data.shape[1] < 2:
        raise ValueError(f"No power trace data found in {csv_path}.")

    trace_time_s = trace_data[:, 0] * 1e-3
    trace_power_w = trace_data[:, 1]
    finite_mask = np.isfinite(trace_time_s) & np.isfinite(trace_power_w)
    trace_time_s = trace_time_s[finite_mask]
    trace_power_w = trace_power_w[finite_mask]

    if len(trace_time_s) == 0:
        raise ValueError(f"No finite power trace samples found in {csv_path}.")

    sort_order = np.argsort(trace_time_s)
    trace_time_s = trace_time_s[sort_order]
    trace_power_w = trace_power_w[sort_order]

    unique_time_s, unique_indices = np.unique(trace_time_s, return_index=True)
    trace_time_s = unique_time_s
    trace_power_w = trace_power_w[unique_indices]

    if np.any(trace_power_w <= 0.0):
        raise ValueError(f"Power trace in {csv_path} contains non-positive values.")

    trace_time_s = trace_time_s - trace_time_s[0]
    return trace_time_s, trace_power_w


def resample_power_trace(trace_time_s, trace_power_w, target_time_s, target_mean_power_w):
    if target_mean_power_w <= 0.0:
        raise ValueError("Target mean laser power must be positive.")

    if len(trace_time_s) == 1:
        return np.full_like(target_time_s, target_mean_power_w), 0.0

    trace_sample_intervals = np.diff(trace_time_s)
    positive_intervals = trace_sample_intervals[trace_sample_intervals > 0.0]

    if len(positive_intervals) == 0:
        raise ValueError("Power trace times must contain at least two distinct values.")

    trace_period_s = trace_time_s[-1] + np.median(positive_intervals)
    target_time_relative_s = target_time_s - target_time_s[0]
    wrapped_time_s = np.mod(target_time_relative_s, trace_period_s)
    interpolation_time_s = np.concatenate((trace_time_s, [trace_period_s]))
    interpolation_power_w = np.concatenate((trace_power_w, [trace_power_w[0]]))
    resampled_power_w = np.interp(
        wrapped_time_s,
        interpolation_time_s,
        interpolation_power_w
    )
    resampled_power_w = (
        resampled_power_w
        - np.mean(resampled_power_w)
        + target_mean_power_w
    )

    if np.any(resampled_power_w <= 0.0):
        raise ValueError(
            "Mean-shifted laser power trace contains non-positive values."
        )

    return resampled_power_w, trace_period_s
...
# ****************************************************************************************************************************************************
# BAOAB solution with Brownian motion and optional measured laser-power noise
# ****************************************************************************************************************************************************
rng = np.random.default_rng(seed=brownian_seed)

if use_laser_power_noise:
    laser_trace_time_s, laser_trace_power_w = load_power_meter_trace(
        laser_noise_trace_path
    )
    laser_noise_source_sample_count = len(laser_trace_power_w)
    laser_noise_source_mean_power = np.mean(laser_trace_power_w)
    laser_power_time, laser_noise_trace_duration_actual = resample_power_trace(
        laser_trace_time_s,
        laser_trace_power_w,
        t_baoab,
        P_laser
    )
    laser_noise_resampled_mean_power = np.mean(laser_power_time)
    laser_power_factor = laser_power_time / P_laser
else:
    laser_noise_source_sample_count = 0
    laser_noise_source_mean_power = P_laser
    laser_noise_resampled_mean_power = P_laser
    laser_noise_trace_duration_actual = 0.0
    laser_power_factor = np.ones_like(t_baoab)
    laser_power_time = P_laser * laser_power_factor

laser_power_factor_min = float(np.min(laser_power_factor))
laser_power_factor_max = float(np.max(laser_power_factor))
laser_equilibrium_power_factors = np.unique(
    np.array([laser_power_factor_min, 1.0, laser_power_factor_max])
)
...
# print("Laser-noise trace path =", laser_noise_trace_path)
# print("Laser-noise source sample count =", laser_noise_source_sample_count)
# print("Laser-noise source mean power =", laser_noise_source_mean_power, "W")
# print("Laser-noise resampled mean power =", laser_noise_resampled_mean_power, "W")
# print("Laser-noise trace duration used =", laser_noise_trace_duration_actual, "s")
# print("Laser-noise power-factor range =", laser_power_factor_min, "to", laser_power_factor_max)
...
plt.plot(t_plot, laser_power_time[plot_slice] * 1e3, label="laser power")
plt.axhline(P_laser * 1e3, linestyle="--", color="black", label="mean power")

if use_laser_power_noise and laser_power_factor_max > laser_power_factor_min:
    plt.axhline(
        np.max(laser_power_time) * 1e3,
        linestyle=":",
        color="tab:red",
        label="trace range"
    )
    plt.axhline(np.min(laser_power_time) * 1e3, linestyle=":", color="tab:red")

plt.xlabel("Time / s")
plt.ylabel("Laser power / mW")
plt.title("Measured laser-power noise")
plt.legend()
plt.grid()
plt.tight_layout()
finish_plot()
